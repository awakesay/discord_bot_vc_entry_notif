...
class Channel:
    ...
    DELETE_STATEMENT: Final[
        str
    ] = f"""
        DELETE FROM {TABLE_NAME}
        WHERE DELETE_KEY = :delete_key;
    """
    SQLITE_MASTER_STATEMENT: Final[
        str
    ] = f"""
    ...
    def get_records_by_voice_channel_id(
        self, voice_channel_id: Union[int, str]
    ) -> list:
    ...
    def get_records_by_guild_id_and_delete_key(
        self, guild_id: Union[int, str], delete_key: str
    ) -> list:
        ...
        self.cursor.execute(
            statement, {"guild_id": str(guild_id), "delete_key": delete_key}
        )
        return self.cursor.fetchall()
    ...
    def _generate_delete_key(self) -> str:
        ...
        statement: str = (
            f"SELECT DELETE_KEY FROM {self.TABLE_NAME} WHERE DELETE_KEY = :delete_key"
        )
        while True:
            generate_delete_key: str = hashlib.sha256(
